...
For this problem, I created some utility methods:
- `new_matrix` - Creates new matrix to memory and returns the pointer to the matrix.
- `free_matrix` - Free memory allocated for the matrix.
- `rand_matrix` - fill the matrix with random integers.
...
I created a simple sequential multiplication we can compare with later.
```c
...
`pt_matrix_mult` receives two matrix arguments and number of threads to utilize.
Then it splits the matrix multiplication task to multiple tasks and assign the sub-computations to differnt threads.
...
```
> clang 34.c csapp.c -o 34 && ./34
... (Compiler output) ...
Testing sequential matrix multiplication...
Sequential matrix multiplication took 2.45 seconds (2453705 microseconds).

Testing threaded matrix multiplication...
[1 threads] 2.42 seconds (2415919 microseconds).
[2 threads] 1.25 seconds (1253823 microseconds).
[4 threads] 0.66 seconds (663750 microseconds).
[8 threads] 0.34 seconds (342980 microseconds).
[16 threads] 0.34 seconds (340123 microseconds).
[32 threads] 0.34 seconds (335562 microseconds).
```
...
| Cores (p)              | 1       | 2       | 4      | 8      | 8      | 8      |
+========================+=========+=========+========+========+========+========+
| Running Time (Tp) [ns] | 2415919 | 1253823 | 663750 | 342980 | 340123 | 335562 |
+------------------------+---------+---------+--------+--------+--------+--------+
| Speed up (Sp)          | 1       | 1.93    | 3.64   | 7.04   | 7.1    | 7.2    |
+------------------------+---------+---------+--------+--------+--------+--------+
| Efficiency (Ep)        | 100%    | 96%     | 90%    | 88%    | 88%    | 89%    |
+------------------------+---------+---------+--------+--------+--------+--------+
```

We can see that adding more than 8 threads doesn't contribute to speed or efficieny.<br/>
This is because I have 8 Cores available on my machine.

## Full program source 34.c
```c
#include "csapp.h"
#include <assert.h>
#include <time.h>
...
int main() {
    ...
    printf("Sequential matrix multiplication took %.2f seconds (%llu nanoseconds).\n", sequentialSecs, sequentialTimeElapsed);
    printf("\n");
    ...
    for (int thread_cnt = 1; thread_cnt <= 32; thread_cnt *= 2) {
        ...
        printf ("[%d threads] %.2f seconds (%llu nanoseconds).\n", thread_cnt, threadedSecs, threadedTimeElapsed);
